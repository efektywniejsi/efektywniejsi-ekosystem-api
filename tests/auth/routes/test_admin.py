...
from tests.utils.helpers import assert_user_response_valid, create_auth_headers
...
class TestCreateUserEndpoint:
    ...
    async def test_should_create_user_when_admin(self, test_client, test_admin_token, db_session):
        ...
        response = await test_client.post("/api/v1/admin/users", json=payload, headers=headers)
...
class TestUpdateUserEndpoint:
    ...
    async def test_should_update_user_name_when_admin(
        ...
        response = await test_client.patch(
            f"/api/v1/admin/users/{test_user.id}", json=payload, headers=headers
        )
    ...
    async def test_should_return_404_when_user_not_found(self, test_client, test_admin_token):
        ...
        response = await test_client.patch(
            f"/api/v1/admin/users/{fake_uuid}", json=payload, headers=headers
        )
