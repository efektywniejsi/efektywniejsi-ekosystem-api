...
from app.catalog.dependencies import require_admin
...
from app.catalog.services import SalesWindowService
...
async def get_next_sales_window(
    ...
    sales_window = (
        ...
        .filter(
            and_(
                SalesWindow.status == SalesWindowStatus.UPCOMING,
...
async def create_sales_window(
    ...
    if window_data.status == "active":
        existing_active = (
            db.query(SalesWindow).filter(SalesWindow.status == SalesWindowStatus.ACTIVE).first()
        )
...
async def update_sales_window(
    ...
    if update_data.status is not None:
        ...
        sales_window.status = SalesWindowStatus(update_data.status)  # type: ignore[assignment]
        updated_fields.append("status")

    # Update startsAt
    if update_data.startsAt is not None:
        sales_window.starts_at = update_data.startsAt  # type: ignore[assignment]
        updated_fields.append("startsAt")

    # Update endsAt
    if update_data.endsAt is not None:
        sales_window.ends_at = update_data.endsAt  # type: ignore[assignment]
    ...
    # Update audit fields
    sales_window.updated_at = datetime.utcnow()  # type: ignore[assignment]
    sales_window.updated_by = str(admin.id)  # type: ignore[assignment]
...
async def delete_sales_window(
    ...
    sales_window.status = SalesWindowStatus.CLOSED  # type: ignore[assignment]
    sales_window.updated_at = datetime.utcnow()  # type: ignore[assignment]
    sales_window.updated_by = str(admin.id)  # type: ignore[assignment]
